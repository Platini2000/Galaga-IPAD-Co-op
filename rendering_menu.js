                ...
                joystickMovedVerticallyLastFrame = currentJoystickMoved;
                 if (crossPressedNow && !crossPressedLast) {
                     ...
                     if (isGameModeSelectMode) {
                        ...
                        if (selectedButtonIndex === 0) { // 1 PLAYER
                            startGame1P();
                        } else { // 2 PLAYER (selectedButtonIndex === 1)
                 ...
                 if (!actionTakenThisFrame && circlePressedNow && !circlePressedLast) {
                      ...
                      if (isGameModeSelectMode) {
                        ...
                        startAutoDemoTimer();
                        actionTakenThisFrame = true;
                      } else { triggerFullscreen(); playSound(menuMusicSound); actionTakenThisFrame = true; } // Key
                 }
            }
        }
...
// showScoreScreen wordt aangepast in deel 2
...
function showMenuState() {
    try {
       ...
       gameJustStartedAndWaveLaunched = false;
       wasLastGameAIDemo = false; initialGameStartSoundPlayedThisSession = false; isInGameState = false; isShowingScoreScreen = false; isPaused = false; isPlayerSelectMode = false; isFiringModeSelectMode = false; isGameModeSelectMode = false; selectedGameMode = 'normal'; isTwoPlayerMode = false; currentPlayer = 1; isShowingPlayerGameOverMessage = false;
    ...
    try {
        ...
        if (isTwoPlayerMode && selectedGameMode === 'coop' && level === 1) {
            ...
            if (isCoopAIDemoActive) {
                isShowingCoopPlayersReady = true;
                coopPlayersReadyStartTime = Date.now();
            } else {
                isShowingCoopPlayersReady = true;
                coopPlayersReadyStartTime = Date.now();
            }
        ...
        } else if (level === 1 && setManualControl) {
             if (!initialGameStartSoundPlayedThisSession) {
...
function handleTouchStart(event) {
    ...
    for (let i = 0; i < event.changedTouches.length; i++) {
        ...
        let touchHandledForThisFinger = false;
...
function renderGame() {
    try {
        ...
        const drawTopUiElement=(label, scoreValue, labelAlign, labelX, shouldBlink = false)=>{
             ...
             if (label === "DEMO" || label === "DEMO-1" || label === "DEMO-2") {
                 ...
                 if (isInGameState && (!isManualControl || isCoopAIDemoActive)) {
                      blinkOnDuration = DEMO_TEXT_BLINK_ON_MS * 0.7; blinkCycleDuration = DEMO_TEXT_BLINK_CYCLE_MS;
                 }
             }
        ...
        if (isTwoPlayerMode) {
            ...
            else if (selectedGameMode === 'normal' && isInGameState) { score2PValue = (currentPlayer === 2) ? score : player2Score; }
        ...
        if (isShowingResultsScreen) {
            ...
            label1P = (wasLastGameAIDemo && !isCoopAIDemoActive) ? "DEMO" : (isCoopAIDemoActive ? "DEMO-1" : "1UP");
            ...
            score1PValue = player1Score || 0; sessionHighScore = highScore || 20000; sessionHighScore = Math.max(sessionHighScore, score1PValue, score2PValue);
            label1P = isCoopAIDemoActive ? "DEMO-1" : "1UP";
            highScoreConditionMet = false; show1UPBlink = false; show2UPBlink = false;
        }
        else if (!isInGameState) {
            score1PValue = 0; sessionHighScore = highScore || 20000; label1P = "1UP"; highScoreConditionMet = false; show1UPBlink = false;
            if (isTwoPlayerMode) score2PValue = 0;
            label2P = "2UP";
            show2UPBlink = false;
        }
        else {
            ...
            if (isCoopAIDemoActive) {
                ...
                score1PValue = player1Score; sessionHighScore = Math.max(highScore, player1Score, player2Score); label1P = "1UP";
                show1UPBlink = baseBlinkCondition && !isShowingIntro && player1Lives > 0 && ship1 && !isPlayer1ShipCaptured && !isPlayer1WaitingForRespawn && !isPlayer1ShowingGameOverMessage;
                show2UPBlink = baseBlinkCondition && !isShowingIntro && player2Lives > 0 && ship2 && !isPlayer2ShipCaptured && !isPlayer2WaitingForRespawn && !isPlayer2ShowingGameOverMessage;
                if (player1Score >= sessionHighScore && player1Score > 0 && !isPlayer1ShowingGameOverMessage) highScoreConditionMet = show1UPBlink;
                if (player2Score >= sessionHighScore && player2Score > 0 && player2Score >= player1Score && !isPlayer2ShowingGameOverMessage) highScoreConditionMet = show2UPBlink;
            } else if (isTwoPlayerMode && selectedGameMode === 'normal') {
                score1PValue = (currentPlayer === 1) ? score : player1Score; sessionHighScore = Math.max(highScore, player1Score, player2Score, score); label1P = "1UP";
                ...
                show2UPBlink = baseBlinkCondition && !isShowingIntro && currentPlayer === 2 && playerLives > 0 && !isShipCaptured && !isWaitingForRespawn && !isShowingPlayerGameOverMessage;
                ...
                score1PValue = score; sessionHighScore = Math.max(sessionHighScore, score); label1P = "1UP";
        ...
        if (highScoreConditionMet) {
            if (!isManualControl || isCoopAIDemoActive) {
                 isHighScoreBlinkingNow = (player1Score >= sessionHighScore && player1Score > 0 && show1UPBlink && !isPlayer1ShowingGameOverMessage) ||
                                       (isCoopAIDemoActive && player2Score >= sessionHighScore && player2Score > 0 && show2UPBlink && !isPlayer2ShowingGameOverMessage);
                 if (!isCoopAIDemoActive && !isManualControl) isHighScoreBlinkingNow = show1UPBlink;
            }
            else if (isTwoPlayerMode && selectedGameMode === 'coop') { isHighScoreBlinkingNow = (player1Score >= sessionHighScore && player1Score > 0 && show1UPBlink && !isPlayer1ShowingGameOverMessage) || (player2Score >= sessionHighScore && player2Score > 0 && show2UPBlink && !isPlayer2ShowingGameOverMessage); }
            else if (isTwoPlayerMode && selectedGameMode === 'normal') { isHighScoreBlinkingNow = (currentPlayer === 1) ? show1UPBlink : show2UPBlink; }
        ...
        const drawLevelIcons = (levelValueToDisplay, isPlayer1_Coop_Or_SinglePlayer) => {
            ...
            let startX;
            const coopLevelIconOffset = 100;

            if (isTwoPlayerMode && selectedGameMode === 'coop' && isInGameState) {
                if (isPlayer1_Coop_Or_SinglePlayer) {
                    startX = LIFE_ICON_MARGIN_LEFT + coopLevelIconOffset;
                } else {
                    startX = gameCanvas.width - LEVEL_ICON_MARGIN_RIGHT - totalWidth - coopLevelIconOffset;
                }
            } else {
        ...
        if (gameOverSequenceStartTime > 0 || isShowingPlayerGameOverMessage || isPlayer1ShowingGameOverMessage || isPlayer2ShowingGameOverMessage) {
            ...
            if (isTwoPlayerMode) levelP2ToDisplay = Math.max(1, player2MaxLevelReached);
        } else if (!isInGameState || isShowingScoreScreen) {
            levelP1ToDisplay = 1;
            if (isTwoPlayerMode) levelP2ToDisplay = 1;
        } else if (isInGameState) {
            if (isTwoPlayerMode && selectedGameMode === 'coop') {
        ...
        if (isTwoPlayerMode && selectedGameMode === 'coop' && levelP2ToDisplay > 0) {
             drawLevelIcons(levelP2ToDisplay, false);
        } else if (isTwoPlayerMode && (!isInGameState || gameOverSequenceStartTime > 0 || isShowingPlayerGameOverMessage || isPlayer1ShowingGameOverMessage || isPlayer2ShowingGameOverMessage) && levelP2ToDisplay > 0){
        ...
        if (drawMenuShip) {
            ...
            if (isTwoPlayerMode && selectedGameMode === 'coop') {
                ...
                if (p1ActiveAndAlive && (!isPlayer1ShipCaptured || (isPlayer1ShipCaptured && !isShowingCaptureMessage))) {
                    ...
                    if (isShowingCSBonusScreen && !p2ActiveAndAlive) {
                    } else if (isShowingCSBonusScreen && p1ActiveAndAlive && p2ActiveAndAlive && isCoopAIDemoActive) {
                ...
                if (p2ActiveAndAlive && (!isPlayer2ShipCaptured || (isPlayer2ShipCaptured && !isShowingCaptureMessage))) {
                    ...
                    if (isShowingCSBonusScreen && !p1ActiveAndAlive) {
                    } else if (isShowingCSBonusScreen && p1ActiveAndAlive && p2ActiveAndAlive && isCoopAIDemoActive) {
                    } else if (isShowingCSBonusScreen && isCoopAIDemoActive) {
                         shouldDrawP2 = false;
                    }
                 ...
                 if (ship && playerLives > 0 && !isShipCaptured && !isShowingCaptureMessage) {
                    ...
                    if (isInvincible || isWaitingForRespawn) {
                     ...
                     if (shouldDrawShip && !(isShowingCSBonusScreen && !isManualControl && !isCoopAIDemoActive )) {
                        let shipDrawX = ship.x;
                        let shouldCenterSingleShipIntro = (isShowingIntro && !isManualControl && selectedGameMode !== 'coop') && !isDualShipActive;
            ...
            if (noGlobalGameOverOrResults || isAnyCoopPlayerGameOverMsgActive) {
                 ...
                 if (!messageDrawnThisCycle) {
                     if (isPaused) { drawCanvasText("PAUSED", midScreenX, midScreenY, PAUSE_TEXT_FONT, PAUSE_TEXT_COLOR, 'center', 'middle', PAUSE_TEXT_SHADOW); messageDrawnThisCycle = true; }
                        ...
                        const coopReadyText = isCoopAIDemoActive ? "DEMO PLAYERS READY!" : "PLAYERS READY!";
                     ...
                     else if (isShowingCaptureMessage) { drawCanvasText("FIGHTER CAPTURED", midScreenX, midScreenY - 40, INTRO_TEXT_FONT, "red", 'center', 'middle', true); messageDrawnThisCycle = true; }
                        ...
                        if (introStep === 1) {
                            if (!(isTwoPlayerMode && selectedGameMode === 'coop' && level === 1)) {
                                introText = (!isManualControl && !isCoopAIDemoActive) ? "DEMO" :
                                            (isCoopAIDemoActive ? "DEMO-1" :
                                            (isTwoPlayerMode ? `PLAYER ${currentPlayer}` : "PLAYER 1"));
                            }
                        }
                        else if (introStep === 2) { introText = "STAGE " + level; }
                        else if (introStep === 3) { introText = "CHALLENGING STAGE"; introColor = INTRO_TEXT_COLOR_CS_TEXT; if (!csIntroSoundPlayed && typeof playSound === 'function') { playSound(entranceSound); csIntroSoundPlayed = true; } } // Key
                        ...
                        if (isPlayer1ShowingGameOverMessage && !isPlayer2ShowingGameOverMessage) {
                            const p1goLabel = isCoopAIDemoActive ? "DEMO-1" : "PLAYER 1";
                            ...
                            const p2goLabel = isCoopAIDemoActive ? "DEMO-2" : "PLAYER 2";
                ...
                if (isShowingPlayerGameOverMessage && selectedGameMode === 'normal') {
                     const playerText = `PLAYER ${playerWhoIsGameOver}`; const lineSpacing = RESULTS_LINE_V_SPACING_SINGLE; const messageCenterY = gameCanvas.height * 0.45; drawCanvasText(playerText, gameCanvas.width / 2, messageCenterY - lineSpacing / 2, INTRO_TEXT_FONT, INTRO_TEXT_COLOR_NORMAL, 'center', 'middle', true); drawCanvasText("GAME OVER", gameCanvas.width / 2, messageCenterY + lineSpacing / 2, INTRO_TEXT_FONT, INTRO_TEXT_COLOR_NORMAL, 'center', 'middle', true);
                }
                    ...
                    if (isShowingGameOverText && !isTwoPlayerMode && !isCoopAIDemoActive) {
                        ...
                        if (isTwoPlayerMode) {
                            const shots1 = player1ShotsFired || 0; const hits1 = player1EnemiesHit || 0; const ratio1 = shots1 > 0 ? Math.round((hits1 / shots1) * 100) + "%" : "0%"; const shots2 = player2ShotsFired || 0; const hits2 = player2EnemiesHit || 0; const ratio2 = shots2 > 0 ? Math.round((hits2 / shots2) * 100) + "%" : "0%";
                            ...
                            const p1Identifier = isCoopAIDemoActive ? "DEMO-1" : "PLAYER 1";
                            const p2Identifier = isCoopAIDemoActive ? "DEMO-2" : "PLAYER 2";
                            drawPlayerResultsColumn(p1Identifier, player1Score, shots1, hits1, ratio1, player1MaxLevelReached, leftColumnX, initialY); drawPlayerResultsColumn(p2Identifier, player2Score, shots2, hits2, ratio2, player2MaxLevelReached, rightColumnX, initialY);

                            let winnerNum = 0;
                            let winnerLabel = "";
                            if (player1Score > player2Score) {
                                winnerNum = 1;
                                winnerLabel = isCoopAIDemoActive ? "DEMO-1" : "PLAYER 1";
                            } else if (player2Score > player1Score) {
                                winnerNum = 2;
                                winnerLabel = isCoopAIDemoActive ? "DEMO-2" : "PLAYER 2";
                            }
                            ...
                            if (winnerNum > 0) {
                             ...
                             const shotsValue = player1ShotsFired || 0; const hitsValue = player1EnemiesHit || 0; const finalScore = (!isManualControl && wasLastGameAIDemo && !isCoopAIDemoActive) ? score : player1Score; const finalLevel = (!isManualControl && wasLastGameAIDemo && !isCoopAIDemoActive) ? level : player1MaxLevelReached; const ratioValue = shotsValue > 0 ? Math.round((hitsValue / shotsValue) * 100) + "%" : "0%";
                             const playerIdentifier = (!isManualControl && wasLastGameAIDemo && !isCoopAIDemoActive) ? "DEMO" : "PLAYER 1";
...
function handleCanvasMouseMove(event) {
    ...
    let currentCursorStyle = 'default'; const isInAnyMenuState = !isInGameState && !isShowingScoreScreen; let hoveringButton = false; let newSelectedButtonIndex = selectedButtonIndex;
    if (isInAnyMenuState) {
        ...
        if (isPlayerSelectMode || isFiringModeSelectMode || isGameModeSelectMode || (!isPlayerSelectMode && !isFiringModeSelectMode && !isGameModeSelectMode && !isShowingScoreScreen)) {
            const button0Rect = getMenuButtonRect(0); const button1Rect = getMenuButtonRect(1);
...
function mainLoop(timestamp) {
    try {
        ...
        if (typeof stopAllGameSoundsInternal === 'function') {
            stopAllGameSoundsInternal(); // This should handle Web Audio sounds correctly
        }
