...
function stopAllGameSoundsInternal() {
    // Met Web Audio API, loopen we door de `soundAssets` om ze te stoppen.
    if (typeof soundAssets === 'object' && soundAssets !== null) {
        Object.keys(soundAssets).forEach(soundNameKey => {
            if (typeof stopSound === 'function') {
                stopSound(soundNameKey); // stopSound beheert nu de Web Audio bronnen
            }
        });
    }
    // isGridSoundPlaying wordt nu intern beheerd door playSound/stopSound voor gridBackgroundSound
}
    ...
    if (isTwoPlayerMode) {
        ...
        if (selectedGameMode === 'coop') {
            ...
            if (gameCanvas && gameCanvas.width > 0 && gameCanvas.height > 0) {
                ...
                const fallbackP1X = 150;
                const fallbackP2X = 250;
                ship1 = { x: fallbackP1X, y: 500, width: SHIP_WIDTH, height: SHIP_HEIGHT, speed: SHIP_MOVE_SPEED, targetX: fallbackP1X, id: 'p1' };
                ship2 = { x: fallbackP2X, y: 500, width: SHIP_WIDTH, height: SHIP_HEIGHT, speed: SHIP_MOVE_SPEED, targetX: fallbackP2X, id: 'p2' };
                if (isCoopAIDemoActive) {
                    smoothedShip1X = fallbackP1X;
                    smoothedShip2X = fallbackP2X;
                    aiNeedsStabilization = false;
                    aiPlayerActivelySeekingCaptureById = null;
                    coopAICaptureDiveAnticipationActive = false;
                    coopAICaptureDiveAnticipationEndTime = 0;
...
function resetWaveInternal() {
    ...
    if (isShowingIntro && !(isTwoPlayerMode && selectedGameMode === 'coop' && level === 1)) {
    ...
    if (selectedGameMode !== 'coop') {
        if (ship && gameCanvas && gameCanvas.width > 0 && gameCanvas.height > 0 && (forceCenterShipNextReset || !isManualControl)) {
            let effectiveWidthForCentering = ship.width;
            if (isDualShipActive) {
                effectiveWidthForCentering = DUAL_SHIP_OFFSET_X + SHIP_WIDTH;
            }
    ...
    } else if (gameCanvas && gameCanvas.width > 0 && gameCanvas.height > 0) {
        ...
        if (isCoopAIDemoActive) {
            if (p1Active && p2Active) {
                const p1InitialX = gameCanvas.width / 2 - gameCanvas.width * COOP_SHIP_HORIZONTAL_OFFSET_FACTOR - (SHIP_WIDTH / 2);
                if(ship1) { ship1.targetX = p1InitialX; smoothedShip1X = p1InitialX; }
                const p2InitialX = gameCanvas.width / 2 + gameCanvas.width * COOP_SHIP_HORIZONTAL_OFFSET_FACTOR - (SHIP_WIDTH / 2);
                if(ship2) { ship2.targetX = p2InitialX; smoothedShip2X = p2InitialX; }
                ...
                ship1.targetX = centeredX1; smoothedShip1X = centeredX1;
                ...
                ship2.targetX = centeredX2; smoothedShip2X = centeredX2;
            }
        } else {
             if (p1Active && ship1) {
                 ship1.y = gameCanvas.height - SHIP_HEIGHT - SHIP_BOTTOM_MARGIN;
             }
             if (p2Active && ship2) {
                 ship2.y = gameCanvas.height - SHIP_HEIGHT - SHIP_BOTTOM_MARGIN;
             }
        }
    }
    forceCenterShipNextReset = false;

    if (!isManualControl && aiNeedsStabilization && selectedGameMode !== 'coop' && !isCoopAIDemoActive) {
...
function startFullGridWave() {
    ...
    // De playSound functie beheert isGridSoundPlaying nu.
    // Als het geluid al speelt (bijv. van een vorige grid wave die niet correct stopte),
    // zal playSound het niet opnieuw starten.
...
function handleEnemyHit(enemy, shootingPlayerId = null) {
    ...
    const now = Date.now();
    let points = 0; let destroyed = false; let wasBossDamagedBeforeHit = enemy.isDamaged; let playHitSoundKey = null; // Changed to key
    ...
    if (enemy.health <= 0) {
        ...
        if (points > 0) {
            ...
            if (!isManualControl) {
                if (isCoopAIDemoActive) {
                    ...
                    } else { // Fallback for CO-OP AI if shooterId is somehow null
                        player1Score += points; playerSpecificEnemiesHitIncrementer = () => player1EnemiesHit++;
                        if (player1Score > highScore) highScore = player1Score;
                        checkAndAwardExtraLife(1);
                    }
                } else { // Standaard 1P AI Demo
                    score += points;
                    player1Score = score;
            ...
            } else if (isTwoPlayerMode && selectedGameMode === 'coop') { // Manual CO-OP
                if (shootingPlayerId === 'player1') {
                    player1Score += points; playerSpecificEnemiesHitIncrementer = () => player1EnemiesHit++;
                    ...
                    if (player2Score > highScore) {
                        ...
                        if (!player2TriggeredHighScoreSound) { player2TriggeredHighScoreSound = true; playSound(hiScoreSound); }
                    }
                    checkAndAwardExtraLife(2);
            ...
            } else { // 1P Manual of 2P Alternating
                score += points;
        ...
        if (enemy.id === capturingBossId) { stopSound(captureSound); } // Key
        if (enemy.state === 'attacking' || enemy.state === 'following_entrance_path' || enemy.state === 'diving_to_capture_position' || enemy.state === 'following_bezier_path' || enemy.state === 'returning' || enemy.state === 'showing_capture_message') {
            if (enemy.type === ENEMY3_TYPE) stopSound(bossGalagaDiveSound); // Key
            else stopSound(butterflyDiveSound); // Key
        }

    } else {
        destroyed = false;
        if (enemy.type === ENEMY3_TYPE) { playHitSoundKey = bossHit1Sound; points = 0; } // Key
        else { points = 0; }
    }

    if (playHitSoundKey) { playSound(playHitSoundKey); } // Key
...
function moveEntities() { // Function definition itself is not changed here, only the catch block
    try {
        if (isPaused) {
             return;
        }
        ...
        // isGridSoundPlaying wordt nu beheerd door playSound/stopSound
        stopSound(gridBackgroundSound); // Key
        isEntrancePhaseActive = false;
        stopSound(entranceSound); // Key
        isShowingPlayerGameOverMessage = false; playerGameOverMessageStartTime = 0; playerWhoIsGameOver = 0; nextActionAfterPlayerGameOver = ''; isShipCaptured = false; captureBeamActive = false; capturingBossId = null;
        stopSound(captureSound); // Key
        stopSound(shipCapturedSound); // Key
        isWaitingForRespawn = false; fallingShips = []; isDualShipActive = false; player1IsDualShipActive = false; player2IsDualShipActive = false; isInvincible = false; invincibilityEndTime = 0; hitSparks = []; if(typeof showMenuState === 'function') showMenuState(); if (mainLoopId) cancelAnimationFrame(mainLoopId); mainLoopId = null; alert("Critical error during entity movement/collision. Returning to menu.");
    }
}
...
function firePlayerBullet(shooterId = null) {
    ...
    // AI demo's roepen deze functie aan met shooterId null (1P) of 'player1'/'player2' (CO-OP AI)
    // Voor handmatige besturing wordt shooterId ook correct meegegeven.
    ...
    let currentShooterPlayerId = shooterId; // Wordt gebruikt voor bullet.playerId
    ...
    // Bepaal het schietende schip en gerelateerde vlaggen
    if (isTwoPlayerMode && selectedGameMode === 'coop') {
        ...
        if (ship && playerLives > 0 && !isShipCaptured) {
            ...
            if (isTwoPlayerMode) { // 2P Normal
                if (currentPlayer === 1) {
                    ...
                    incrementPlayerShotsFired = (count) => { player2ShotsFired += count; }; useSingleShotFlag = selectedFiringMode === 'single';
            ...
            if (!currentShooterPlayerId && isTwoPlayerMode) currentShooterPlayerId = `player${currentPlayer}`; // Voor manual 2P normal
            else if (!currentShooterPlayerId) currentShooterPlayerId = 'player1'; // Voor manual 1P
            canShootLogic = true;
        }
    }

    // AI Demo (1P) Specifiek geval, als shooterId null is
    if (shooterId === null && !isManualControl && !isCoopAIDemoActive && ship && playerLives > 0 && !isShipCaptured && !isShowingCaptureMessage) {
        ...
        useSingleShotFlag = false; // AI gebruikt geen single shot mode
    ...
    // Cooldown & Single Shot Logic
    if (useSingleShotFlag) {
        let fireButtonIsCurrentlyPressed = false;
        if (currentShooterPlayerId === 'player1') fireButtonIsCurrentlyPressed = p1FireInputWasDown;
        else if (currentShooterPlayerId === 'player2') fireButtonIsCurrentlyPressed = p2FireInputWasDown;
        // Voor AI, is er geen 'fireButtonIsCurrentlyPressed' op dezelfde manier. AI timing wordt anders beheerd.
        // Als het AI is, is useSingleShotFlag sowieso false.

        if (getSingleShotFlag()) { // Was er net geschoten in single mode?
            if (fireButtonIsCurrentlyPressed && isManualControl) return false; // Knop nog steeds ingedrukt door speler, niet opnieuw vuren
            else setSingleShotFlag(false); // Knop losgelaten, of AI, reset vlag
        }
    } else { // Rapid fire
    ...
    try {
        ...
        playSound(playerShootSound); // Key
...
function handlePlayerInput() {
     try {
         const now = Date.now();
         // isSingleShotMode wordt nu binnen firePlayerBullet gebruikt, niet direct hier.

         const inCoopLevel1IntroStrict = isTwoPlayerMode && selectedGameMode === 'coop' && level === 1 &&
                                       coopPlayersReadyStartTime > 0 && now < coopPlayersReadyStartTime + 8000;

         if (isCoopAIDemoActive) { // AI demo bestuurt zichzelf
             leftPressed = false; rightPressed = false; shootPressed = false;
             p2LeftPressed = false; p2RightPressed = false; p2ShootPressed = false;
             return;
         }
        ...
        // Stap 1: Lees fysieke controller inputs
        let ctrlP1Left = false, ctrlP1Right = false, ctrlP1ShootIsRaw = false; // Ruwe input van primaire controller
        let ctrlP2Left = false, ctrlP2Right = false, ctrlP2ShootIsRaw = false; // Ruwe input van secundaire controller
        ...
        // Stap 2: Update pXFireInputWasDown (fysieke knop ingedrukt JA/NEE voor deze frame)
        // Dit is nodig voor de single-shot logica in firePlayerBullet.
        p1FireInputWasDown = keyboardP1ShootDown || ctrlP1ShootIsRaw;
        p2FireInputWasDown = keyboardP2ShootDown || ctrlP2ShootIsRaw; // Voor P2's eigen inputs

        // Stap 3: Bepaal de actieve `leftPressed`, `rightPressed`, `shootPressed` voor de huidige speler(s)
        leftPressed = false; rightPressed = false; shootPressed = false;
        p2LeftPressed = false; p2RightPressed = false; p2ShootPressed = false;
        ...
        if (isTwoPlayerMode && selectedGameMode === 'coop') {
            ...
            if (p1CanControl) {
                ...
                shootPressed = !inCoopLevel1IntroStrict && (keyboardP1ShootDown || ctrlP1ShootIsRaw);
            ...
            if (p2CanControl) {
                if (connectedGamepadIndexP2 !== null && connectedGamepadIndexP2 !== connectedGamepadIndex) { // P2 heeft eigen controller
                    ...
                    p2ShootPressed = !inCoopLevel1IntroStrict && (keyboardP2ShootDown || ctrlP2ShootIsRaw);
                    // p2FireInputWasDown is al correct gezet.
                } else { // P2 gebruikt P2 keyboard (P1 controller wordt hier NIET voor P2 gebruikt)
                    p2LeftPressed = keyboardP2LeftDown;
                    p2RightPressed = keyboardP2RightDown;
                    p2ShootPressed = !inCoopLevel1IntroStrict && keyboardP2ShootDown;
                    // p2FireInputWasDown is al correct gezet (zal alleen P2 keyboard reflecteren).
                }
            ...
            if (activePlayerCanControl) {
                if (currentPlayer === 1) {
                    leftPressed = keyboardP1LeftDown || ctrlP1Left;
                    rightPressed = keyboardP1RightDown || ctrlP1Right;
                    shootPressed = !isShowingIntro && (keyboardP1ShootDown || ctrlP1ShootIsRaw);
                    // p1FireInputWasDown is correct. p2FireInputWasDown is false voor P1's beurt.
                    if (p2FireInputWasDown && !(connectedGamepadIndexP2 !== null && connectedGamepadIndexP2 !== connectedGamepadIndex && keyboardP2ShootDown )) {
                       // Als P2 geen eigen controller heeft en P2 keyboard niet gebruikt,
                       // maar p2FireInputWasDown (ctrlP2ShootRaw) was true, dan is P1 ctrl gebruikt.
                       // Dit is dubbelop met p1FireInputWasDown. We zorgen dat P2's input hier niet meetelt.
                       p2FireInputWasDown = false;
                    }

                } else { // currentPlayer === 2
                    let p2EffectiveCtrlLeft = false, p2EffectiveCtrlRight = false, p2EffectiveCtrlShoot = false;
                    if (connectedGamepadIndexP2 !== null && connectedGamepadIndexP2 !== connectedGamepadIndex) { // P2 heeft EIGEN controller
                        p2EffectiveCtrlLeft = ctrlP2Left;
                        p2EffectiveCtrlRight = ctrlP2Right;
                        p2EffectiveCtrlShoot = ctrlP2ShootIsRaw;
                        p2FireInputWasDown = keyboardP2ShootDown || ctrlP2ShootIsRaw; // Herbevestig voor P2
                    } else if (connectedGamepadIndex !== null) { // P2 gebruikt P1's controller
                        p2EffectiveCtrlLeft = ctrlP1Left;
                        p2EffectiveCtrlRight = ctrlP1Right;
                        p2EffectiveCtrlShoot = ctrlP1ShootIsRaw;
                        p2FireInputWasDown = ctrlP1ShootIsRaw; // P2 gebruikt P1 controller's fysieke staat
                    }
                    // Als geen controllers, dan zijn p2EffectiveCtrl... false, en P2 keyboard wordt gebruikt.
                    // keyboardP2LeftDown etc. zijn de fysieke P2 keyboard states.

                    leftPressed = keyboardP2LeftDown || p2EffectiveCtrlLeft;
                    rightPressed = keyboardP2RightDown || p2EffectiveCtrlRight;
                    shootPressed = !isShowingIntro && (keyboardP2ShootDown || p2EffectiveCtrlShoot);
                    p1FireInputWasDown = false; // P1 is niet aan de beurt
                }
        ...
        // Stap 4: Vuur kogels af (alleen als manual control)
        if (isManualControl) {
            if (isTwoPlayerMode && selectedGameMode === 'coop') {
                ...
                if (ship2 && player2Lives > 0 && !isPlayer2ShipCaptured && !isPlayer2WaitingForRespawn && !isPlayer2ShowingGameOverMessage && !player2NeedsRespawnAfterCapture && p2ShootPressed) {
...
// Deze blijven hieronder ongewijzigd ten opzichte van de vorige versie
// (die de 2s vertraging voor CO-OP AI Demo correct had)

 function aiControl() {
    try {
        if (isCoopAIDemoActive) return;

        if (isPaused || isManualControl || playerLives <= 0 || !ship || !gameCanvas || !isInGameState || gameOverSequenceStartTime > 0 || isShowingPlayerGameOverMessage || isShipCaptured || isShowingCaptureMessage) {
        ...
        if (smoothedShipX === undefined && ship) {
            smoothedShipX = ship.x;
        } else if (smoothedShipX === undefined && !ship) {
            smoothedShipX = gameCanvas ? gameCanvas.width / 2 : 0;
            return;
        }
        ...
        if (isDualShipActive) {
            effectiveShipWidth = DUAL_SHIP_OFFSET_X + ship.width;
        }
        const shipCenterX = ship.x + effectiveShipWidth / 2;
        ...
        let desiredTargetX = smoothedShipX;
        let shouldTryShoot = false;
        let isDodgingThreat = false;
        let dodgeTargetX = smoothedShipX;
        let targetEnemyForAI = null;
        let isMovingToCapture = false;

        const ignoreCaptureMode = isDualShipActive;
        ...
        if (now < aiStabilizationEndTime && !isShowingBlockingMessage) {
            ...
            if (!isInvincible) {
                ...
                const dangerZoneForBullets = { x: ship.x - bulletBuffer, y: ship.y - bulletLookahead, width: effectiveShipWidth + bulletBuffer * 2, height: bulletLookahead + ship.height };

                for (const bullet of enemyBullets) {
                    if (bullet && bullet.y + bullet.height > ship.y - ship.height*2 && bullet.y < canvasHeight) {
                ...
                if (threateningBullets.length > 0) {
                    ...
                    for (let dodgeDir = -1; dodgeDir <= 1; dodgeDir += 2) {
                        ...
                        let potentialDodgeX = smoothedShipX + dodgeDir * dodgeAmount;
                        ...
                        const testDodgeZone = { x: potentialDodgeX - bulletBuffer/2, y: ship.y - bulletLookahead, width: effectiveShipWidth + bulletBuffer, height: bulletLookahead + ship.height};
                        ...
                        if (Math.abs(potentialDodgeX - smoothedShipX) < effectiveShipWidth * 0.5) dodgeScore -=10;
                ...
                if (!isDodgingThreat) {
                    ...
                    for (const currentEnemy of enemies) {
                        if (currentEnemy && (currentEnemy.state === 'attacking' || currentEnemy.state === 'diving_to_capture_position' || currentEnemy.state === 'following_entrance_path' || currentEnemy.state === 'following_bezier_path') && currentEnemy.y + currentEnemy.height > ship.y - enemyLookahead/2 && currentEnemy.y < ship.y + ship.height) {
                            const dangerZoneForEnemy = { x: ship.x - enemyBuffer, y: ship.y - enemyLookahead, width: effectiveShipWidth + enemyBuffer * 2, height: enemyLookahead + ship.height };
                            ...
                            if (checkCollision(dangerZoneForEnemy, enemyRect)) {
                                ...
                                dodgeTargetX = smoothedShipX + dodgeDirection * dodgeAmount;
                                break;
                            }
                        }
                    }
            ...
            if (isDodgingThreat) {
            ...
            } else if (fallingShips.length > 0 && !isShipCaptured && !isWaitingForRespawn && !isDualShipActive ) {
                const fallingShip = fallingShips[0]; if (fallingShip) { const targetFallCenterX = fallingShip.x + fallingShip.width / 2; desiredTargetX = targetFallCenterX - (effectiveShipWidth / 2); desiredTargetX = Math.max(AI_EDGE_BUFFER, Math.min(canvasWidth - effectiveShipWidth - AI_EDGE_BUFFER, desiredTargetX)); targetEnemyForAI = null; let bestCatchTargetScore = -Infinity; let catchTargetEnemy = null; for (const currentEnemy of enemies) { if (currentEnemy && currentEnemy.y < shipTopY) { const enemyCenterX = currentEnemy.x + currentEnemy.width / 2; const alignmentDiff = Math.abs(enemyCenterX - shipCenterX); const verticalDistance = shipTopY - (currentEnemy.y + currentEnemy.height); if (alignmentDiff < effectiveShipWidth * 1.5 && verticalDistance > 0) { const currentScore = (canvasHeight - currentEnemy.y) - alignmentDiff * 2 - verticalDistance; if (currentScore > bestCatchTargetScore) { bestCatchTargetScore = currentScore; catchTargetEnemy = currentEnemy; } } } } if (catchTargetEnemy) { let shootPathClearOfBullets = true; const dangerZoneForCatchShoot = { x: ship.x - ENTRANCE_SHOOT_BULLET_CHECK_BUFFER / 2, y: ship.y - ENTRANCE_SHOOT_BULLET_CHECK_LOOKAHEAD, width: effectiveShipWidth + ENTRANCE_SHOOT_BULLET_CHECK_BUFFER, height: ENTRANCE_SHOOT_BULLET_CHECK_LOOKAHEAD + ship.height }; for (const bullet of enemyBullets) { if (bullet && bullet.y + bullet.height > 0 && bullet.y < canvasHeight) { const bulletRect = { x: bullet.x, y: bullet.y, width: bullet.width, height: bullet.height }; if (checkCollision(dangerZoneForCatchShoot, bulletRect)) { shootPathClearOfBullets = false; break; } } } if (shootPathClearOfBullets && !(catchTargetEnemy.type === ENEMY3_TYPE && (catchTargetEnemy.state === 'preparing_capture' || catchTargetEnemy.state === 'diving_to_capture_position' || catchTargetEnemy.state === 'capturing'))) { shouldTryShoot = true; targetEnemyForAI = catchTargetEnemy; } } }
            } else {
                let shouldConsiderCapture = false;
                let capturingBoss = null;
                isMovingToCapture = false;

                if (!ignoreCaptureMode && !isManualControl && captureBeamActive && capturingBossId && playerLives > 1 && !isShipCaptured && !isWaitingForRespawn) {
                    capturingBoss = enemies.find(e => e.id === capturingBossId);
                    if (capturingBoss && capturingBoss.state === 'capturing') {
                        shouldConsiderCapture = true;
                        const timeSinceCaptureStart = now - capturingBoss.captureStartTime;
                        if (timeSinceCaptureStart >= AI_CAPTURE_WAIT_DURATION_MS) {
                            isMovingToCapture = true;
                            const bossCenterX = capturingBoss.x + capturingBoss.width / 2;
                ...
                if (!isMovingToCapture) {
                    targetEnemyForAI = null;
                    if (isEntrancePhaseActive && !isChallengingStage && !isFullGridWave) {
                        desiredTargetX = targetCenterShipX;
                        let bestEntranceTargetScore = -Infinity;
                        let potentialTarget = null;
                        for (const currentEnemy of enemies) {
                            if (currentEnemy && (currentEnemy.state === 'following_entrance_path' || currentEnemy.state === 'moving_to_grid') && currentEnemy.y < shipTopY) {
                                if (ignoreCaptureMode && currentEnemy.type === ENEMY3_TYPE && (currentEnemy.state === 'preparing_capture' || currentEnemy.state === 'diving_to_capture_position' || currentEnemy.state === 'capturing')) continue;
                                const enemyCenterX = currentEnemy.x + currentEnemy.width / 2;
                                const enemyCenterY = currentEnemy.y + currentEnemy.height / 2;
                                const dx = enemyCenterX - shipCenterX;
                                const dy = shipTopY - enemyCenterY;
                                const distance = Math.sqrt(dx*dx + dy*dy);
                                const alignmentScore = Math.max(0, (effectiveShipWidth * 1.5) - Math.abs(dx));
                                const currentScore = (canvasHeight - currentEnemy.y) * 1.5 + alignmentScore - distance * 0.5;
                                if (currentScore > bestEntranceTargetScore) { bestEntranceTargetScore = currentScore; potentialTarget = currentEnemy; }
                            }
                        }
                        if (potentialTarget) {
                            targetEnemyForAI = potentialTarget;
                            const targetCenterX = targetEnemyForAI.x + targetEnemyForAI.width / 2;
                            const alignmentDiff = Math.abs(shipCenterX - targetCenterX);
                            const alignmentThreshold = effectiveShipWidth * ENTRANCE_SHOOT_ALIGNMENT_FACTOR;
                            let shootPathClearOfBullets = true;
                            const dangerZoneForEntranceShoot = { x: ship.x - ENTRANCE_SHOOT_BULLET_CHECK_BUFFER / 2, y: ship.y - ENTRANCE_SHOOT_BULLET_CHECK_LOOKAHEAD, width: effectiveShipWidth + ENTRANCE_SHOOT_BULLET_CHECK_BUFFER, height: ENTRANCE_SHOOT_BULLET_CHECK_LOOKAHEAD + ship.height };
                            for (const bullet of enemyBullets) { if (bullet && bullet.y + bullet.height > 0 && bullet.y < canvasHeight) { const bulletRect = { x: bullet.x, y: bullet.y, width: bullet.width, height: bullet.height }; if (checkCollision(dangerZoneForEntranceShoot, bulletRect)) { shootPathClearOfBullets = false; break; } } }
                            if (shootPathClearOfBullets && alignmentDiff < alignmentThreshold) {
                                if (targetEnemyForAI.type === ENEMY3_TYPE && (targetEnemyForAI.state === 'preparing_capture' || targetEnemyForAI.state === 'diving_to_capture_position' || targetEnemyForAI.state === 'capturing')) {
                                     shouldTryShoot = false;
                                } else if (isDualShipActive || targetEnemyForAI.type !== ENEMY3_TYPE || captureAttemptMadeThisLevel || isShipCaptured) {
                                    shouldTryShoot = true;
                                }
                            }
                        }
                    } else if (isChallengingStage) {
                        let bestTargetScore = Infinity; targetEnemyForAI = null;
                        for (const currentEnemy of enemies) {
                            if (currentEnemy && currentEnemy.state === 'following_bezier_path' && currentEnemy.y < shipTopY) {
                                const enemyCenterX = currentEnemy.x + currentEnemy.width / 2;
                                const alignmentDiff = Math.abs(enemyCenterX - shipCenterX);
                                const verticalPriority = (1.0 - Math.min(1.0, currentEnemy.y / shipTopY)) * 2.0;
                                const horizontalPriority = (alignmentDiff / canvasWidth) * 0.5;
                                const currentScore = verticalPriority + horizontalPriority;
                                if (currentScore < bestTargetScore) { bestTargetScore = currentScore; targetEnemyForAI = currentEnemy; }
                            }
                        }
                        let aimX_CS = shipCenterX;
                        if (targetEnemyForAI) {
                            const enemyCenterX = targetEnemyForAI.x + targetEnemyForAI.width / 2;
                            const enemyCenterY = targetEnemyForAI.y + targetEnemyForAI.height / 2;
                            const dyTarget = shipTopY - enemyCenterY;
                            let predictedTargetX_CS = enemyCenterX;
                            const isHorizontalSquadron = (targetEnemyForAI.squadronId === 2 || targetEnemyForAI.squadronId === 3);
                            if (!isHorizontalSquadron && dyTarget > 0 && PLAYER_BULLET_SPEED > 0.1) { const timeToReachApproxCS = Math.min(MAX_PREDICTION_TIME_CS, dyTarget / PLAYER_BULLET_SPEED); const adjustedPredictionFactor = 0.4; const predictedOffset = (targetEnemyForAI.velocityX || 0) * timeToReachApproxCS; predictedTargetX_CS = enemyCenterX + predictedOffset * adjustedPredictionFactor; predictedTargetX_CS = Math.max(targetEnemyForAI.width / 2, Math.min(canvasWidth - targetEnemyForAI.width / 2, predictedTargetX_CS)); }
                            aimX_CS = predictedTargetX_CS;
                            desiredTargetX = aimX_CS - effectiveShipWidth / 2;
                            desiredTargetX = Math.max(AI_EDGE_BUFFER, Math.min(canvasWidth - effectiveShipWidth - AI_EDGE_BUFFER, desiredTargetX));
                            const CS_SHOOT_ALIGNMENT_THRESHOLD_EXTRA_WIDE = effectiveShipWidth * 1.5;
                            const currentAlignmentDiff = Math.abs(shipCenterX - aimX_CS);
                            if (currentAlignmentDiff < CS_SHOOT_ALIGNMENT_THRESHOLD_EXTRA_WIDE) { shouldTryShoot = true; }
                        } else { const wiggleOffset = Math.sin(now / AI_WIGGLE_PERIOD * 2 * Math.PI) * AI_WIGGLE_AMPLITUDE * 0.5; desiredTargetX = targetCenterShipX + wiggleOffset; desiredTargetX = Math.max(AI_EDGE_BUFFER, Math.min(canvasWidth - effectiveShipWidth - AI_EDGE_BUFFER, desiredTargetX)); }
                    } else {
                        let bestTargetScore = -Infinity; targetEnemyForAI = null;
                        const isWaitingForCaptureButNotMoving = shouldConsiderCapture && !isMovingToCapture;
                        for (const currentEnemy of enemies) {
                            if (!currentEnemy) continue;
                            if (isWaitingForCaptureButNotMoving && currentEnemy.id === capturingBossId) { continue; }
                            if (ignoreCaptureMode && currentEnemy.type === ENEMY3_TYPE && (currentEnemy.state === 'preparing_capture' || currentEnemy.state === 'diving_to_capture_position' || currentEnemy.state === 'capturing')) continue;

                            let isValidTarget = false; let basePriority = 0;
                            switch(currentEnemy.state) {
                                case 'in_grid': isValidTarget = true; basePriority = isWaitingForCaptureButNotMoving ? 20000 : 18000; break;
                                case 'moving_to_grid': case 'returning': isValidTarget = true; basePriority = 9000; break;
                                case 'preparing_attack': isValidTarget = true; basePriority = 7000; break;
                                case 'attacking': isValidTarget = true; basePriority = 6000; break;
                                case 'preparing_capture': isValidTarget = true; basePriority = 25000; break;
                            }
                            if (isValidTarget) {
                                let currentScore = basePriority;
                                let damageBonus = (currentEnemy.isDamaged && currentEnemy.type === ENEMY3_TYPE) ? 5000 : 0;
                                let yPriority = (canvasHeight - currentEnemy.y) * 2;
                                let xAlignmentBonus = Math.max(0, (effectiveShipWidth * 1.0) - Math.abs((currentEnemy.x + currentEnemy.width/2) - shipCenterX)) * 1;
                                currentScore += yPriority + damageBonus + xAlignmentBonus;
                                if (currentScore > bestTargetScore) { bestTargetScore = currentScore; targetEnemyForAI = currentEnemy; }
                            }
                        }
                        let aimX = targetCenterShipX + effectiveShipWidth / 2;
                        if (targetEnemyForAI) {
                            const enemyCenterX = targetEnemyForAI.x + targetEnemyForAI.width / 2;
                            const enemyCenterY = targetEnemyForAI.y + targetEnemyForAI.height / 2;
                            const dy = shipTopY - enemyCenterY;
                            let predictedTargetX = enemyCenterX;
                            if (targetEnemyForAI.state !== 'in_grid' && targetEnemyForAI.state !== 'preparing_capture') { if (dy > 0 && PLAYER_BULLET_SPEED > 0.1) { const timeToReachApprox = Math.min(MAX_PREDICTION_TIME, dy / PLAYER_BULLET_SPEED); let enemyPredictedXCalc = enemyCenterX + (targetEnemyForAI.velocityX || 0) * timeToReachApprox; predictedTargetX = Math.max(targetEnemyForAI.width / 2, Math.min(canvasWidth - targetEnemyForAI.width / 2, enemyPredictedXCalc)); } }
                            aimX = predictedTargetX;
                            desiredTargetX = aimX - effectiveShipWidth / 2;
                            if (targetEnemyForAI.state === 'in_grid' || targetEnemyForAI.state === 'preparing_capture') { desiredTargetX = Math.max(AI_EDGE_BUFFER, Math.min(canvasWidth - effectiveShipWidth - AI_EDGE_BUFFER, desiredTargetX)); }
                            ...
                            let shotBlockedByEdge = false;
                            let horizontalDiff = Math.abs(shipCenterX - aimX);
                            let alignmentThreshold = (targetEnemyForAI.state === 'in_grid' || targetEnemyForAI.state === 'preparing_capture') ? effectiveShipWidth * GRID_SHOOT_ALIGNMENT_FACTOR : effectiveShipWidth * FINAL_SHOOT_ALIGNMENT_THRESHOLD;
                            const shipNearLeftEdge = ship.x < AI_EDGE_BUFFER * AI_EDGE_SHOOT_BUFFER_FACTOR;
                            const shipNearRightEdge = ship.x > canvasWidth - effectiveShipWidth - AI_EDGE_SHOOT_BUFFER_FACTOR;
                            const targetPastCenterRight = aimX > canvasWidth * AI_EDGE_SHOOT_TARGET_THRESHOLD_FACTOR;
                            const targetPastCenterLeft = aimX < canvasWidth * (1.0 - AI_EDGE_SHOOT_TARGET_THRESHOLD_FACTOR);
                            if ((shipNearLeftEdge && targetPastCenterRight) || (shipNearRightEdge && targetPastCenterLeft)) { shotBlockedByEdge = true; }

                            if (!shotBlockedByEdge && horizontalDiff < alignmentThreshold) {
                                let shotBlockedByEnemy = false;
                                if (targetEnemyForAI.state !== 'in_grid' && targetEnemyForAI.state !== 'preparing_capture') { for (const otherEnemy of enemies) { if (!otherEnemy || otherEnemy === targetEnemyForAI) continue; if (isWaitingForCaptureButNotMoving && otherEnemy.id === capturingBossId) continue; if (otherEnemy.y < shipTopY && otherEnemy.y + otherEnemy.height > targetEnemyForAI.y) { const otherLeft = otherEnemy.x - AI_SHOT_CLEARANCE_BUFFER; const otherRight = otherEnemy.x + otherEnemy.width + AI_SHOT_CLEARANCE_BUFFER; if (aimX > otherLeft && aimX < otherRight) { shotBlockedByEnemy = true; break; } } } }
                                if (!shotBlockedByEnemy) {
                                    if (targetEnemyForAI.type === ENEMY3_TYPE && (targetEnemyForAI.state === 'preparing_capture' || targetEnemyForAI.state === 'diving_to_capture_position' || targetEnemyForAI.state === 'capturing')) {
                                        shouldTryShoot = false;
                                    } else if (!isWaitingForCaptureButNotMoving || targetEnemyForAI.id !== capturingBossId) {
                                        shouldTryShoot = true;
                                    }
                                }
                            }
                        } else { const wiggleOffset = Math.sin(now / AI_WIGGLE_PERIOD * 2 * Math.PI) * AI_WIGGLE_AMPLITUDE; desiredTargetX = targetCenterShipX + wiggleOffset; desiredTargetX = Math.max(AI_EDGE_BUFFER, Math.min(canvasWidth - effectiveShipWidth - AI_EDGE_BUFFER, desiredTargetX)); }
                    }
                }
            }
        }

        if (typeof smoothedShipX === 'number') {
             smoothedShipX += (desiredTargetX - smoothedShipX) * AI_SMOOTHING_FACTOR_MOVE;
        } else {
             smoothedShipX = desiredTargetX;
        }
        ship.targetX = smoothedShipX;

        if (shouldTryShoot && !isDodgingThreat && !isMovingToCapture && !isShowingBlockingMessage) {
            // AI_SHOOT_COOLDOWN wordt in firePlayerBullet afgehandeld via aiLastShotTime
             if (now >= aiCanShootTime && !isShowingIntro) { // aiCanShootTime is een extra globale cooldown voor AI
                firePlayerBullet(null); // shooterId null voor 1P AI
            }
        }
    } catch (e) {
 ...
 /**
 * Controls CO-OP AI ships (demo mode).
 */
function aiControlCoop() {
    if (!isCoopAIDemoActive || isPaused || !gameCanvas || !isInGameState || gameOverSequenceStartTime > 0 ) {
    ...
    const getShipBlockingState = (shipIdForBlockCheck) => {
        ...
        return isGameOverForThisShip ||
               ...
               (isShowingIntro && !(level === 1 && coopPlayersReadyStartTime > 0 && isCoopAIDemoActive));
    };
    ...
    if (coopAICaptureDiveAnticipationActive && now < coopAICaptureDiveAnticipationEndTime) {
        p1ShouldShootOverrideGeneral = false;
    ...
    if (p1CanAct && ship1) {
        ...
        if (sts1 && p1ShouldShootOverrideGeneral && !(isShowingCaptureMessage && isPlayer1ShipCaptured) ) {
            if (p1IsSavingPartner || now >= aiShip1CanShootTime) {
                ...
                // aiShip1LastShotTime wordt in fireCoopAIBullet gezet
    ...
    if (p2CanAct && ship2) {
        ...
        if (sts2 && p2ShouldShootOverrideGeneral && !(isShowingCaptureMessage && isPlayer2ShipCaptured) ) {
            if (p2IsSavingPartner || now >= aiShip2CanShootTime) {
                fireCoopAIBullet(ship2, player2IsDualShipActive, 'player2');
                // aiShip2LastShotTime wordt in fireCoopAIBullet gezet
...
function fireCoopAIBullet(shootingShip, isDual, shooterPlayerId) {
    ...
    if (shooterPlayerId === 'player2' && isPlayer2ShowingGameOverMessage) shooterIsGameOver = true;
    ...
    if (shooterPlayerId === 'player2' && isPlayer2ShipCaptured) return false;
    ...
    try {
        ...
        if (shooterPlayerId === 'player1') {
            ...
            player1LastShotTime = now; // Gebruik de algemene P1 last shot time, ook voor AI
            aiShip1LastShotTime = now;
        } else if (shooterPlayerId === 'player2') {
            player2ShotsFired += bulletsCreated;
            player2LastShotTime = now; // Gebruik de algemene P2 last shot time, ook voor AI
...
function calculateAIDesiredState(currentShip, currentSmoothedX, isShipDual, gameEnemies, gameEnemyBullets, gameFallingShips, isShipInvincible, isThisShipCaptured, isThisShipWaitingForRespawn, currentTime, gameCanvasWidth, isCurrentShipCompletelyBlocked, shipIdentifier) {
    ...
    const livesOfThisAIShip = (shipIdentifier === 'p1') ? player1Lives : player2Lives;
    ...
    if (isCurrentShipCompletelyBlocked || isThisShipCaptured || isThisShipWaitingForRespawn || (shipIdentifier === 'p1' && player1NeedsRespawnAfterCapture) || (shipIdentifier === 'p2' && player2NeedsRespawnAfterCapture) ) {
        desiredTargetX = targetCenterShipX;
        if (isThisShipCaptured || isThisShipWaitingForRespawn || (shipIdentifier === 'p1' && player1NeedsRespawnAfterCapture) || (shipIdentifier === 'p2' && player2NeedsRespawnAfterCapture) ) {
    ...
    if (otherPlayerIsActuallyCaptured && !otherPlayerIsShowingGameOver && capturedBossIdWithMessage) {
        let allowTargetingCapturedBoss = true;
        if (isCoopAIDemoActive) {
    ...
    if (!isShipInvincible) {
        let threateningBullets = [];
        const bulletLookahead = FINAL_DODGE_LOOKAHEAD * (isCoopAIDemoActive ? 1.6 : 0.8);
        const bulletBuffer = FINAL_DODGE_BUFFER_BASE * (isCoopAIDemoActive ? 1.7 : 0.8);
        ...
        if (threateningBullets.length > 0) {
            ...
            const dodgeAmountBase = effectiveShipWidth * (isCoopAIDemoActive ? 3.0 : 2.0);
        ...
        if (!isDodgingThreat) {
            const enemyLookahead = AI_COLLISION_LOOKAHEAD * (isCoopAIDemoActive ? 1.0 : 0.75);
            const enemyCollisionBuffer = AI_COLLISION_BUFFER * (isCoopAIDemoActive ? 1.0 : 0.8);
            for (const enemy of gameEnemies) {
                ...
                if ((enemy.state === 'attacking' || enemy.state === 'diving_to_capture_position') &&
                    ...
                    if (checkCollision(dangerZoneForEnemy, enemyRect)) {
                        ...
                        desiredTargetX = currentSmoothedX + ((shipCenterX < enemyCenterX) ? -1 : 1) * (effectiveShipWidth * (isCoopAIDemoActive ? 2.2 : 1.9));
                        break;
                    }
                }
            }
    ...
    if (!isDodgingThreat && !isTargetingCapturedShipBoss && !isShipDual && captureBeamActive && capturingBossId && !isThisShipCaptured) {
    ...
    if (!isDodgingThreat && !isTargetingCapturedShipBoss && !isMovingToCaptureBeam && !isShipDual) {
    ...
    if (!isDodgingThreat && !isMovingToCaptureBeam && !isMovingForOwnFallingShip && !isTargetingCapturedShipBoss) {
        ...
        for (const enemy of gameEnemies) {
            ...
            if (enemy.type === ENEMY3_TYPE && enemy.hasCapturedShip && livesOfThisAIShip <= 1) {
                 const isPartnerBoss = enemy.id === capturedBossIdWithMessage && otherPlayerIsActuallyCaptured;
                 if (!isPartnerBoss) {
            ...
            else if (shipIdentifier === 'p2' && enemyCenterX >= gameCanvasWidth / 2) laneBonus = 5000;
        ...
        if (isCoopAIDemoActive && isShipDual && targetEnemyForAI) {
        ...
        if (targetEnemyForAI) {
            ...
            if (targetEnemyForAI.type === ENEMY3_TYPE && targetEnemyForAI.hasCapturedShip && livesOfThisAIShip <= 1) {
                const isPartnerBoss = targetEnemyForAI.id === capturedBossIdWithMessage && otherPlayerIsActuallyCaptured;
                if (!isPartnerBoss) {
...
function triggerImmediateCaptureDive() {
    try {
        ...
        if (potentialCapturingBosses.length > 0) {
            ...
            playSound(bossGalagaDiveSound); // Key
...
function moveEntities() {
    ...
    try {
            ...
            if (isManualControl) {
                if (isTwoPlayerMode && selectedGameMode === 'coop') {
                    ...
                    if (ship && playerLives > 0 && !isShipCaptured && !isShowingPlayerGameOverMessage && gameOverSequenceStartTime === 0) {
                        ...
                        else if (rightPressed && ship.x < gameCanvas.width - effectiveWidth) ship.x += ship.speed;
                        ship.targetX = ship.x;
                    }
                }
            } else { // AI Control (Demo modes)
                if (isCoopAIDemoActive) {
                    if (ship1 && player1Lives > 0 && !isPlayer1ShipCaptured && !isPlayer1WaitingForRespawn && !isPlayer1ShowingGameOverMessage && !player1NeedsRespawnAfterCapture) {
                ...
                } else if (ship) { // Standaard 1P AI Demo
                    if (playerLives > 0 && !isShipCaptured && !isShowingPlayerGameOverMessage && gameOverSequenceStartTime === 0) {
                        ...
                        if (Math.abs(moveDifference) > AI_MOVEMENT_DEADZONE) ship.x += Math.sign(moveDifference) * Math.min(Math.abs(moveDifference), effectiveShipSpeed);
            ...
            if (!fs.landed) {
                ...
                if (fs.y >= TARGET_FALLING_SHIP_Y) {
                    ...
                    if (rescuerIsActiveAndExists && !rescuerAlreadyDual) {
                        setDualShipForRescuer();
                        playSound(dualShipSound); // Key
                        if (playerShipIsCurrentlyCaptured && !isCoopAIDemoActive) {
            ...
            if (!fs.landed) {
                ...
                if (targetShipForDocking && !isTargetShipDualAlready) {
                    ...
                    if (checkCollision(fallingShipRect, playerShipRect)) {
                        setDualShipFlagEarly();
                        if (playerShipCapturedStateForEarlyDock && !isCoopAIDemoActive) {
                            playSound(dualShipSound); // Key
        ...
        // Grid Movement
        let gridHorizontalShift = 0; const gridEnemiesPresent = enemies.some(e => e?.state === 'in_grid'); const gridShouldBeMoving = !isChallengingStage && !isWaveTransitioning && gridEnemiesPresent && !isShowingPlayerGameOverMessage && !(isTwoPlayerMode && selectedGameMode === 'coop' && (isPlayer1ShowingGameOverMessage || isPlayer2ShowingGameOverMessage));
        if (gridShouldBeMoving) {
            if (!isGridSoundPlaying) { // isGridSoundPlaying is now managed by playSound/stopSound
                playSound(gridBackgroundSound); // Key
            }
            const gridEnemiesList = enemies.filter(e => e?.state === 'in_grid');
            if (gridEnemiesList.length > 0) {
                let minX = gameCanvas.width, maxX = 0;
                gridEnemiesList.forEach(enemy => { if (enemy) { minX = Math.min(minX, enemy.x); maxX = Math.max(maxX, enemy.x + enemy.width); } });
                ...
                gridHorizontalShift = effectiveGridMoveSpeed * gridMoveDirection; currentGridOffsetX += gridHorizontalShift;
                enemies.forEach(e => { if (e && (e.state === 'returning' || e.state === 'in_grid' || e.state === 'moving_to_grid')) { try { const enemyWidthForGrid = (e.type === ENEMY3_TYPE) ? BOSS_WIDTH : ((e.type === ENEMY1_TYPE) ? ENEMY1_WIDTH : ENEMY_WIDTH); const { x: newTargetX, y: newTargetY } = getCurrentGridSlotPosition(e.gridRow, e.gridCol, enemyWidthForGrid); e.targetGridX = newTargetX; e.targetGridY = newTargetY; } catch (gridPosError) { console.error(`Error updating target grid pos for enemy ${e?.id} (state: ${e?.state}) during shift:`, gridPosError); } } });
            }
        } else {
            if (isGridSoundPlaying) { // isGridSoundPlaying is now managed by playSound/stopSound
                stopSound(gridBackgroundSound); // Key
            ...
            if (enemy && !isShowingPlayerGameOverMessage && !(isTwoPlayerMode && selectedGameMode === 'coop' && (isPlayer1ShowingGameOverMessage || isPlayer2ShowingGameOverMessage)) && gameOverSequenceStartTime === 0) {
                ...
                if (collisionStates.includes(enemy.state)) {
                    ...
                    if (isTwoPlayerMode && selectedGameMode === 'coop') {
                        ...
                        if (ship && playerLives > 0 && !isShipCaptured && !isShowingPlayerGameOverMessage && (!isManualControl ? !isInvincible : true)) {
 ...
 * <<< GEWIJZIGD: COOP AI DEMO: Starttijd van partner capture bijhouden. >>>
 */
function handlePlayerShipCollision(playerNumber, hitDualPart, collisionTime, wasCapturedHit = false) {
    ...
    if (isTwoPlayerMode && selectedGameMode === 'coop') {
        ...
        if (shipObjectToUpdate) {
            ...
            if (wasCapturedHit) {
             ...
             else if (isCoopAIDemoActive && gameCanvas) {
    ...
    if (isTwoPlayerMode && selectedGameMode === 'coop') {
        if (playerNumber === 1) {
            if (wasCapturedHit) {
                ...
                player1IsDualShipActive = false;
                player1Lives--;
                if (isCoopAIDemoActive) coopPartner1CapturedTime = now;

                if (player1Lives <= 0) {
                    player1Lives = 0;
                    isPlayer1ShowingGameOverMessage = true;
                    player1GameOverMessageStartTime = now;
                    playSound(gameOverSound); // Key
                    ship1 = null;
                    player1NeedsRespawnAfterCapture = false;
            ...
            if (wasCapturedHit) {
                ...
                player2IsDualShipActive = false;
                player2Lives--;
                if (isCoopAIDemoActive) coopPartner2CapturedTime = now;
                ...
                if (player2Lives <= 0) {
                    ...
                    ship2 = null;
                    player2NeedsRespawnAfterCapture = false;
                    if (aiPlayerActivelySeekingCaptureById === 'p2') aiPlayerActivelySeekingCaptureById = null;
                    if (capturedBossIdWithMessage && enemies.find(e => e.id === capturedBossIdWithMessage)) {
                        const boss = enemies.find(e => e.id === capturedBossIdWithMessage);
                        if (boss) { boss.hasCapturedShip = false; boss.capturedShipDimensions = null; }
                    }
    ...
    } else { // 1P of 2P Alternating (Normal modes)
        if (isDualShipActive && !wasCapturedHit && shipObjectToUpdate) {
            isDualShipActive = false;
            if (playerNumber === 1 && isTwoPlayerMode) player1IsDualShipActive = false;
            else if (playerNumber === 2 && isTwoPlayerMode) player2IsDualShipActive = false;
            else if (!isTwoPlayerMode) player1IsDualShipActive = false;
            ...
            if (!isManualControl) aiNeedsStabilization = true;
        } else {
            ...
            if (playerLives <= 0) {
                ...
                if (isTwoPlayerMode) { if (playerNumber === 1) player1Lives = playerLives; else player2Lives = playerLives; }

                if (!wasCapturedHit) {
                    isInvincible = true; invincibilityEndTime = now + INVINCIBILITY_DURATION_MS;
                    isWaitingForRespawn = true; respawnTime = now + RESPAWN_DELAY_MS;
                    if (ship && gameCanvas) { ship.x = lastKnownX; ship.y = shipBaseY; ship.targetX = ship.x; if (!isManualControl) aiNeedsStabilization = true; }
                } else {
                }
    ...
    try {
        ...
        if(isManualControl && connectedGamepadIndex !== null){
            if(isTwoPlayerMode && selectedGameMode === 'coop'){
                ...
                if(playerLives > 0 && !isShipCaptured && !isShowingPlayerGameOverMessage) primaryControllerCanExit = true;
            }
        }
        ...
        if(isManualControl && isTwoPlayerMode && selectedGameMode === 'coop' && connectedGamepadIndexP2 !== null){
        ...
        if (isShowingCSBonusScreen && gameCanvas?.width > 0) {
            if (isTwoPlayerMode && selectedGameMode === 'coop') {
                ...
                if (p1Active && p2Active) {
                    ...
                    if(isCoopAIDemoActive) smoothedShip1X = centeredX1;
                    if(ship2 && isCoopAIDemoActive) { ship2.targetX = ship2.x; smoothedShip2X = ship2.x; }
                } else if (p2Active && ship2) {
                    const p2EffectiveWidth = player2IsDualShipActive ? (SHIP_WIDTH + DUAL_SHIP_OFFSET_X) : SHIP_WIDTH;
                    const centeredX2 = Math.round(gameCanvas.width / 2 - p2EffectiveWidth / 2);
                    ship2.x = centeredX2; ship2.targetX = centeredX2;
                    if(isCoopAIDemoActive) smoothedShip2X = centeredX2;
                    if(ship1 && isCoopAIDemoActive) { ship1.targetX = ship1.x; smoothedShip1X = ship1.x; }
                }
            } else if (!isManualControl && !isCoopAIDemoActive && ship) {
        ...
        if (isTwoPlayerMode && selectedGameMode === 'coop' && level === 1 && coopPlayersReadyStartTime > 0) {
            ...
            if (!isCoopAIDemoActive && !isShowingCoopPlayersReady && !isShowingIntro && (now >= coopPlayersReadyStartTime + 4000)) {
                isShowingIntro = true; introStep = 2; introDisplayStartTime = now;
                explosions = []; if(typeof updateExplosions === 'function') updateExplosions();
            }

            if (isShowingIntro && introStep === 2) {
                 if(now - introDisplayStartTime < 100) { explosions = []; if(typeof updateExplosions === 'function') updateExplosions(); }
                const stage1EndTime = coopPlayersReadyStartTime + 8000;
                if (now >= stage1EndTime) {
                    isShowingIntro = false; introStep = 0; playerIntroSoundPlayed = false; stageIntroSoundPlayed = false; csIntroSoundPlayed = false;
                    explosions = []; if(typeof updateExplosions === 'function') updateExplosions();
                }
            }

            if (isManualControl || isCoopAIDemoActive) handlePlayerInput();
            if (typeof moveEntities === 'function') moveEntities();
            renderGame();

            if (isShowingCoopPlayersReady || (isShowingIntro && introStep === 2) || now < coopPlayersReadyStartTime + 8000) {
                return;
            } else {
        ...
        if (isTwoPlayerMode && selectedGameMode === 'coop' && level === 1 && !coopLevel1IntroIsCurrentlyActive && !gameJustStartedAndWaveLaunched && coopPlayersReadyStartTime === 0) {
            ...
            if (now - playerGameOverMessageStartTime >= PLAYER_GAME_OVER_MESSAGE_DURATION) {
                ...
                if (nextActionAfterPlayerGameOver === 'switch_player') {
                    if (switchPlayerTurn()) {
                        ...
                        resetWaveInternal(); gameJustStartedAndWaveLaunched = false;
                ...
                const shipBaseY = gameCanvas ? gameCanvas.height - SHIP_HEIGHT - SHIP_BOTTOM_MARGIN : 500;
                 if (isTwoPlayerMode && selectedGameMode === 'coop') {
                    if (player1NeedsRespawnAfterCapture && player1Lives > 0) {
                        ...
                        player1NeedsRespawnAfterCapture = false;
                        player1CaptureRespawnX = 0;
                        if (isCoopAIDemoActive) smoothedShip1X = ship1.x;
                    }
                    if (player2NeedsRespawnAfterCapture && player2Lives > 0) {
                        ...
                        player2NeedsRespawnAfterCapture = false;
                        player2CaptureRespawnX = 0;
                        if (isCoopAIDemoActive) smoothedShip2X = ship2.x;
                    }

                    if ((player1Lives <=0 && player2Lives <=0) ){
                        if(gameOverSequenceStartTime === 0) triggerFinalGameOverSequence();
                    }
                 } else if (!isTwoPlayerMode && !isManualControl) {
        ...
        const inNormalIntro = isShowingIntro && !coopLevel1IntroIsCurrentlyActive && !(isTwoPlayerMode && selectedGameMode === 'coop' && level ===1);
        ...
        if (!coopLevel1IntroIsCurrentlyActive && gameOverSequenceStartTime === 0 && activePlayersForLogic && !isShowingCaptureMessage && noPlayerGameOverIsActive) {
            if (!isManualControl && isInGameState && !inNormalIntro) {
                if (!(isShowingCSBonusScreen && !isManualControl)) {
                    if (isCoopAIDemoActive) {
                        if (coopAICaptureDiveAnticipationActive && now < coopAICaptureDiveAnticipationEndTime) {
                            aiControlCoop();
                        } else {
                            if (coopAICaptureDiveAnticipationActive) {
                                coopAICaptureDiveAnticipationActive = false;
                                coopAICaptureDiveAnticipationEndTime = 0;
                            }
                            aiControlCoop();
        ...
        if (isCoopAIDemoActive && isChallengingStage && enemies.length === 0 && !isCsCompletionDelayActive && !showCsBonusScoreMessage && !showPerfectMessage && gameCanvas && gameCanvas.width > 0) {
        ...
        if (isCsCompletionDelayActive && noPlayerGameOverIsActive && gameOverSequenceStartTime === 0) {
             if (now - csCompletionDelayStartTime >= CS_COMPLETION_MESSAGE_DELAY + EXPLOSION_DURATION) {
                 ...
                 if (csCompletionResultIsPerfect) {
                     playSound(csPerfectSound); // Key
                     showCsHitsMessage = true;
                     csHitsMessageStartTime = now;
                 } else {
                     playSound(csClearSound); // Key
        ...
        if (inNormalIntro && noPlayerGameOverIsActive && gameOverSequenceStartTime === 0) {
            let introTextFinished = false; const elapsedIntroTime = now - introDisplayStartTime;
            const playerIntroDuration = (isTwoPlayerMode && selectedGameMode === 'normal' && !isCoopAIDemoActive) ? TWO_PLAYER_STAGE_INTRO_DURATION : INTRO_DURATION_PER_STEP;
            let currentIntroStepDuration = introStep === 1 ? playerIntroDuration : INTRO_DURATION_PER_STEP;

            if (introStep === 1) {
                if(elapsedIntroTime < 100) { explosions = []; if(typeof updateExplosions === 'function') updateExplosions(); }
                let soundToPlayKey = null;
                if (level === 1 && !isTwoPlayerMode && !initialGameStartSoundPlayedThisSession && !playerIntroSoundPlayed) {
                    soundToPlayKey = startSound;
                } else if (level === 1 && isTwoPlayerMode && selectedGameMode === 'normal' && !initialGameStartSoundPlayedThisSession && !playerIntroSoundPlayed) {
                    soundToPlayKey = startSound;
                } else if (isTwoPlayerMode && selectedGameMode === 'normal' && !playerIntroSoundPlayed && !isCoopAIDemoActive) {
                    soundToPlayKey = levelUpSound;
                }

                if(soundToPlayKey) {
                    playSound(soundToPlayKey);
                    playerIntroSoundPlayed = true;
                    if (soundToPlayKey === startSound) {
                ...
                if (elapsedIntroTime >= currentIntroStepDuration) {
                    if (isChallengingStage && level > 1) introStep = 3;
                    else introStep = 2;
                ...
                if (!stageIntroSoundPlayed) {
                    if (playLevelUpAfterCSBonus && (isTwoPlayerMode && selectedGameMode === 'coop')) {
                        playSound(levelUpSound); // Key
                        playLevelUpAfterCSBonus = false;
                    } else if (level > 1 || (level === 1 && !isTwoPlayerMode && !initialGameStartSoundPlayedThisSession && !playerIntroSoundPlayed)) {
                         if (level > 1 || (level === 1 && isTwoPlayerMode && selectedGameMode === 'normal' && !playerIntroSoundPlayed && !isCoopAIDemoActive && initialGameStartSoundPlayedThisSession)) {
                            playSound(levelUpSound); // Key
                         }
                         else if (level === 1 && !isManualControl && !isCoopAIDemoActive && !stageIntroSoundPlayed) {
                              if(!initialGameStartSoundPlayedThisSession){ playSound(startSound); initialGameStartSoundPlayedThisSession = true; coopStartSoundPlayedThisSession = true; } // Key
                         }
                    }
                ...
                if(elapsedIntroTime < 100) { explosions = []; if(typeof updateExplosions === 'function') updateExplosions(); }
                if (!csIntroSoundPlayed) { playSound(entranceSound); csIntroSoundPlayed = true; } // Key
                if (elapsedIntroTime >= currentIntroStepDuration) {
                    introTextFinished = true;
                    explosions = []; if(typeof updateExplosions === 'function') updateExplosions();
                }
            ...
            if (isInGameState && noPlayerGameOverIsActive && gameOverSequenceStartTime === 0) {
                if (isManualControl) handlePlayerInput();
                if (typeof moveEntities === 'function') moveEntities();
                updateExplosions(); updateFloatingScores();
            }
            ...
            if (introTextFinished) {
                ...
                if (!isChallengingStage) {
                    if (isCoopAIDemoActive) {
                        ...
                        aiCanShootTime = Date.now() + 3000;
                    }
                } else {
                     if (isCoopAIDemoActive) { aiShip1CanShootTime = 0; aiShip2CanShootTime = 0;}
                     else aiCanShootTime = 0;
                }
                const isNonCoopL1 = level === 1 && !(isTwoPlayerMode && selectedGameMode === 'coop');
                const shouldLaunchWaveNow = (isNonCoopL1 && !gameJustStartedAndWaveLaunched) || level > 1 || isChallengingStage;
                ...
                if (shouldLaunchWaveNow) {
                    if (isChallengingStage) startChallengingStageSequence();
                        ...
                        if (currentWaveDefinition && currentWaveDefinition.length > 0) {
                            if (isFullGridWave) startFullGridWave();
                            else { scheduleEntranceFlightWave(); playSound(entranceSound); } // Key
                        } else {
                            isEntrancePhaseActive = false; stopSound(entranceSound); isWaveTransitioning = true; readyForNextWaveReset = true; bullets = []; enemyBullets = []; explosions = []; // Key
                            setTimeout(() => {
                                ...
                                if ((isInGameState || (!isInGameState && livesCheck)) && typeof resetWaveInternal === 'function') {
                                    if(livesCheck) { resetWaveInternal(); gameJustStartedAndWaveLaunched = false; }
                                    else triggerFinalGameOverSequence();
                                }
                            }, 100);
                        }
                    }
                    if (isNonCoopL1 || level > 1 || isChallengingStage) gameJustStartedAndWaveLaunched = true;
                }
            }
            renderGame(); return;
        }
        ...
        if (gameJustStarted && noSpecialOrNormalIntroRunning && level === 1 && !(isTwoPlayerMode && selectedGameMode === 'coop')) {
        ...
        if (gameJustStarted && noSpecialOrNormalIntroRunning) gameJustStarted = false;
        ...
        if (noSpecialOrNormalIntroRunning && isInGameState && noPlayerGameOverIsActive && gameOverSequenceStartTime === 0) {
            ...
            } else if (isShowingCSBonusScreen && isCoopAIDemoActive) {
                 updateExplosions();
                 updateFloatingScores();
            }
        }
        ...
        if (noSpecialOrNormalIntroRunning && noPlayerGameOverIsActive && gameOverSequenceStartTime === 0) {
            ...
            if (!isWaveTransitioning && activePlayersForLogic && !inNormalIntro && !isShowingCaptureMessage && !isEntrancePhaseActive) {
                ...
                if (waveConsideredComplete) {
                    ...
                    if (isCoopAIDemoActive && gameCanvas && gameCanvas.width > 0) {
                    ...
                    isWaveTransitioning = true; if (isGridSoundPlaying) { stopSound(gridBackgroundSound); } // Key, isGridSoundPlaying zal false worden door stopSound
                    if (!isChallengingStage) {
                        bullets = [];
                        enemyBullets = [];
                    }
                    ...
                    if (isChallengingStage) {
                         ...
                         if (csCompletionResultIsPerfect) {
                             ...
                             if (isTwoPlayerMode && selectedGameMode === 'coop') {
                                 if (player1Lives > 0 || (isCoopAIDemoActive && ship1)) {
                                     player1Score += perfectBonus;
                                 }
                                 if (player2Lives > 0 || (isCoopAIDemoActive && ship2)) {
                     ...
                     setTimeout(() => {
                         ...
                         if (isTwoPlayerMode && selectedGameMode === 'normal') {
                             ...
                             advanceLevel = true;
                             playerToStartNext = (isTwoPlayerMode && selectedGameMode === 'coop') ? 0 : 1;
                         }
                ...
                enemies.forEach(enemy => { if (enemy && enemy.type === ENEMY3_TYPE && enemy.hasCapturedShip && enemy.state === 'attacking' && enemy.capturedShipDimensions && typeof enemy.capturedShipLastFiredTime === 'number') { if (now - enemy.capturedShipLastFiredTime > CAPTURED_SHIP_FIRE_COOLDOWN_MS) { const capturedShipCenterX = enemy.x + enemy.width / 2; const capturedShipBottomY = enemy.y + enemy.height + enemy.capturedShipDimensions.height * 0.5; const firePos = { x: capturedShipCenterX, y: capturedShipBottomY }; if (createBulletSimple(enemy, firePos)) { enemy.capturedShipLastFiredTime = now; playSound(playerShootSound); } } } }); // Key
            }
        } else if (gameOverSequenceStartTime > 0) {
            ...
            if (isShowingResults && !isShowingResultsScreen) {
                isShowingResultsScreen = true; stopSound(gameOverSound); playSound(resultsMusicSound); // Key, Key
                explosions = []; if(typeof updateExplosions === 'function') updateExplosions();
            }
        }
        ...
        if (!coopLevel1IntroIsCurrentlyActive) {
         ...
         alert("A critical error occurred in the game loop. Please refresh.");
         stopAllGameSoundsInternal(); // This will handle stopping all Web Audio sounds
         isInvincible = false; invincibilityEndTime = 0;
 ...
 */
function checkAndAwardExtraLife(playerNumber = null) {
    try {
        ...
        } else if (playerNumber === 2 && isTwoPlayerMode) {
        ...
        } else if (isTwoPlayerMode && selectedGameMode === 'coop' && playerNumber === null) {
        ...
        } else if (!isTwoPlayerMode && (playerNumber === null || playerNumber === 1)) {
             playersToCheck.push({ num: 1, currentLives: playerLives, currentScore: score, thresholdsMet: player1LifeThresholdsMet });
        } else if (isTwoPlayerMode && selectedGameMode === 'normal' && playerNumber === currentPlayer) {
             if (currentPlayer === 1) {
                playersToCheck.push({ num: 1, currentLives: playerLives, currentScore: score, thresholdsMet: player1LifeThresholdsMet });
             } else {
        ...
        playersToCheck.forEach(playerData => {
            ...
            while (true) {
                 ...
                 if (nextThreshold !== -1 && playerData.currentScore >= nextThreshold && !playerData.thresholdsMet.has(nextThreshold)) {
                     if (playerData.num === 1) {
                         if(isTwoPlayerMode && selectedGameMode === 'coop') player1Lives++; else playerLives++;
                         player1Lives = (isTwoPlayerMode && selectedGameMode === 'coop') ? player1Lives : playerLives;
                         playerData.currentLives = player1Lives;
                     } else if (playerData.num === 2) {
                         if(isTwoPlayerMode && selectedGameMode === 'coop') player2Lives++; else playerLives++;
                         player2Lives = (isTwoPlayerMode && selectedGameMode === 'coop') ? player2Lives : playerLives;
